...
class EventScorer:
    ...
    def process_all_games(self) -> Dict:
        predictions_dir = self.file_manager.processed_frames_dir
    ...
    def _generate_game_visualizations(self, game_id: str, summary: Dict):
        ...
        if summary['scored_events']:
            ...
            timestamps = [event['timestamp'] for event in summary['scored_events']]
            ...
            plt.plot(timestamps, scores, 'b-', label='Cumulative Score')
            
            for i, (timestamp, score, event_type) in enumerate(zip(timestamps, scores, event_types)):
                if event_type == 'home_run':
                    color = 'red' 
                else:
                    color = 'green'
                plt.scatter(timestamp, score, c=color, marker='o')
    ...
    def _generate_summary_visualizations(self, summary: Dict, results: Dict[str, Dict]):
        ...
        plt.boxplot([scores for scores in event_scores.values() if scores],
                   labels=[event_type for event_type, scores in event_scores.items() if scores])
